...
class DataCleaner:
    ...
    def encode_with_hashing(self, df, x, n_components=8):
        ...
        hash_encoder = ce.HashingEncoder(
            n_components=n_components, hash_method="sha256", cols=[x]
        )
    ...
    def add_service_history_flag(self):
        self.df["has_service_history"] = [
            1 if x is not None else 0 for x in self.df.service_history
        ]
    ...
    def add_website_flag(self):
        self.df["has_website"] = [
            1 if x is not None else 0 for x in self.df.sellerwebsite
        ]
    ...
    def add_convertible_flag(self):
        self.df["is_convertible"] = [
            1 if x == "Convertible" else 0 for x in self.df.body_type
        ]
    ...
    def add_manufactured_year(self):
        self.df["year_of_manufacture"] = [
            self._extract_manufactured_year(x) for x in self.df.manufactured_year
        ]

    def add_private_plate_flag(self):
        try:
            self.df["is_private_plate"] = [
                self._is_private_plate(reg, year)
                for reg, year in zip(self.df.vrm, self.df.year_of_manufacture)
            ]
        except Exception:
    ...
    def add_mileage_deviation(self):
        self.df["mileage_deviation_encoded"] = [
            self._encode_mileage_deviation(x) for x in self.df.mileageDeviation
        ]

    def group_care_makes(self):
        rare_labels = self._find_rare(self.df.make)
        self.df["make_grouped"] = [
            "rare" if x in rare_labels else self._remove_whitespace_and_symbols(x)
            for x in self.df.make
        ]

    def add_manual_flag(self):
        self.df["is_manual"] = [
            0 if x == "Automatic" else 1 for x in self.df.transmission
        ]
    ...
    def group_fuel_types(self):
        ...
        self.df["fuel_type_grouped"] = [
            "other"
            if x in fuel_rare_labels
            else _remove_plugin_from_fuel(self._remove_whitespace_and_symbols(x))
            for x in self.df.fuel_type
        ]

    def group_doors(self):
        self.df["doors"].fillna("unknown", inplace=True)
        door_rare_labels = self._find_rare(self.df.doors)
        self.df["doors_grouped"] = [
            ">5 doors" if x in door_rare_labels and x != "unknown" else x
            for x in self.df.doors
        ]

    def group_seats(self):
        self.df["seats"].fillna("unknown", inplace=True)
        seat_rare_labels = self._find_rare(self.df.seats)
        self.df["seats_grouped"] = [
            ">6 seats" if x in seat_rare_labels and x != "unknown" else x
            for x in self.df.seats
        ]

    def mileage_to_integer(self):
        self.df["mileage"] = [
            self._extract_integer(r"mile[s]?", x) for x in self.df.mileage
        ]

    def group_owners(self):
        self.df["owners"] = [
            self._extract_integer(r"owner[s]?", x) for x in self.df.owners
        ]
        self.df["owners_grouped"] = [
            self._group_by_num_owners(x) for x in self.df.owners
        ]

    def group_engine_size(self):
        self.df["engine_size_grouped"] = [
            self.categorise_engine_size(x) for x in self.df.engine_size
        ]
    ...
    def group_co2(self):
        self.df["co2_grouped"] = [
            self.categorise_co2_emissions(x) for x in self.df.co2Emissions
        ]

    def count_images(self):
        self.df["image_count"] = [
            x.count("https") if isinstance(x, str) else 0 for x in self.df.images
        ]
    ...
    def extract_lat_long_from_postcode(self):
        ...
        self.df["long_lat_feature"] = [
            self._single_pt_haversine(lat, long, decimal_degrees=True)
            for long, lat in zip(geo_df.longitude, geo_df.latitude)
        ]

    def group_annual_tax(self):
        self.df["annual_tax_grouped"] = [
            self.group_on_quartile(x, "annual_tax") for x in self.df.annual_tax
        ]

    def encode_adverttitle(self):
        self.df["advert_title_sentiment"] = [
            self.get_sentiment(x, self.sid_obj) for x in self.df.adverttitle
        ]

    def encode_advert(self):
        self.df["advert_sentiment"] = [
            self.get_sentiment(x, self.sid_obj) for x in self.df.advert
        ]
    ...
    def clean_data(self):
        ...
        self.df = self.encode_with_hashing(
            self.df, "model", n_components=16
        )  # takes about 30s
        self.outlier_df = self.df.loc[self.df["price"] > 100000]
        self.df = self.df.loc[self.df["price"] <= 100000]  # normal data under 100k
        self.outlier_df, extreme_outliers = self.remove_skewed_outliers(
            self.outlier_df, "price"
        )
        ...
        print(
            f"Percentage of tota ingest dataset: {(num_of_outliers / self.original_size) * 100:.2f}%"
        )
        print(f"Number of extreme outliers removed: {num_of_extreme_outliers}")
        print(
            f"Percentage of tota ingest dataset: {(num_of_extreme_outliers / self.original_size) * 100:.2f}%"
        )
    ...
    def _get_ordinal(self, x, X_train, y_train):
        ...
        med_dict_sorted = {
            k: v for k, v in sorted(med_dict.items(), key=lambda item: item[1])
        }
        label = 0
